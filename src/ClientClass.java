...
public class ClientClass extends JFrame {
    ...
    public ClientClass() {
        ...
        int totalWindowWidth = 3*gameButtonSize+(2+1)*horizontalMargin;
        int totalWindowHeight = 4*gameButtonSize+4*verticalMargin+2*heightOfLabel;
        ...
        labelForPlayer.setFont(new Font(
                "Arial",
                Font.PLAIN,
                (int)(0.7*heightOfLabel) ));
        labelForPlayer.setBounds(
                0,
                verticalMargin,
                totalWindowWidth,
                heightOfLabel);
        ...
        labelAboutOpponent.setFont(new Font(
                "Arial",
                Font.PLAIN,
                (int)(0.7*heightOfLabel) ));
        labelAboutOpponent.setBounds(
                0,
                verticalMargin+heightOfLabel,
                totalWindowWidth,
                heightOfLabel);
        ...
        for (a = 0; a < 3; a++) {
            for (b = 0; b < 3; b++) {
                ...
                buttons[a][b].setFont(new Font(
                        "Arial",
                        Font.PLAIN,
                        (int)(0.7*gameButtonSize) ));
                ...
                buttons[a][b].setBounds(
                        b*gameButtonSize+horizontalMargin,
                        a*gameButtonSize+2*verticalMargin+2*heightOfLabel,
                        gameButtonSize,
                        gameButtonSize
                        ); //x y width height
        ...
        buttonToExit.setFont(new Font(
                "Arial",
                Font.PLAIN,
                (int)(0.7*exitButtonHeight) ));
        buttonToExit.setVisible(false); //for now
        buttonToExit.setEnabled(false); //for now
        buttonToExit.setBounds(
                horizontalMargin+gameButtonSize/2,
                3*gameButtonSize+3*verticalMargin+2*heightOfLabel,
                exitButtonWidth,
                exitButtonHeight
                );
